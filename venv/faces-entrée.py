...
fichier = open(str(maintenant.day) + '-' + str(maintenant.month) + '-' + str(maintenant.year) + '-entrée.txt', 'a')
...
cascade_eye = cv2.CascadeClassifier(r'C:\Users\starinfo\Desktop\Camcheck\venv\haarcascade_eye.xml')
cascade_smile = cv2.CascadeClassifier(r'C:\Users\starinfo\Desktop\Camcheck\venv\haarcascade_smile.xml')
...
while (True):
    ...
    eye = cascade_eye.detectMultiScale(gray, 1.5, 5)
    for (x_eye, y_eye, w_eye, h_eye) in eye:
        cv2.rectangle(frame, (x_eye, y_eye), (x_eye + w_eye, y_eye + h_eye), (0, 180, 60), 2)

    smile = cascade_smile.detectMultiScale(gray, 1.7, 20)
    ...
    for (x, y, w, h) in faces:
        ...
        if conf >= 70:
            ...
            fichier.write("Mm/Mr " + labels[id_] + " a entré à l'entreprise à " + str(maintenant.hour) + " heures " + str(maintenant.minute) + " minutes.")
            fichier.write('\n')
            print(labels[id_])
        else:
            font = cv2.FONT_HERSHEY_SIMPLEX
            ...
            img_item = "inconnu entre" + str(maintenant.hour) + " h" + str(maintenant.minute) + " m.png"
            cv2.imwrite(img_item, roi_color)
            color = (255, 0, 0)
            stroke = 2
            cv2.rectangle(frame, (x, y), (x + w, y + h), color, stroke)
            fichier.write(
                "Mm/Mr " + "Inconnu" + " a entré à l'entreprise à " + str(maintenant.hour) + " heures " + str(
                    maintenant.minute) + " minutes.")
            fichier.write('\n')
            print("Inconnu")
...
fichier.close()
file = open(str(maintenant.day) + '-' + str(maintenant.month) + '-' + str(maintenant.year) + '-entrée.txt', "r")
...
fichier = open(str(maintenant.day) + '-' + str(maintenant.month) + '-' + str(maintenant.year) + '-entrée.txt', 'w')
for i in new_list:
    fichier.write(i)
    fichier.write('\n')
